		...
		for ($i = 0; $i < count($unfocus_chat) - 1; $i++) 
		{
			if (!in_array($unfocus_chat[$i], $double_check) AND !empty($unfocus_chat[$i])) 
			{
				...
				if (!empty($chat))
				{
					...
					$settings .= 'uc_name["' . $unfocus_chat[$i] . '"] = "' . addslashes(strip_tags($chat['username'])) . '";';
					$settings .= 'uc_status["' . $unfocus_chat[$i] . '"] = "' . $chat['status'] . '";';
					$settings .= 'uc_avatar["' . $unfocus_chat[$i] . '"] = "' . addslashes($avatar) . '";';
					$settings .= 'uc_link["' . $unfocus_chat[$i] . '"] = "' . addslashes($link) . '";';
					$settings .= 'uc_message["' . $unfocus_chat[$i] . '"] = "' . addslashes(get_recent_message($chat['userid'])) . '";';
				}
			...
			if (!in_array($value, $double_check2) AND !empty($value)) 
			{
				...
				else
				{
					...
					if ($result AND $db->count_select() > 0) 
					{
						...
						$settings .= 'chatroom_name["' . $value . '"] = "' . addslashes(strip_tags($row['name'])) . '";';
						$double_check2[] = $value;
						$i++;
					}
					else
