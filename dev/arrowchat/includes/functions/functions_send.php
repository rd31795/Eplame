<?php
	...
	*/
	function sanitize($text) 
	{
		...
		{
			$text = str_replace($match[0], '<span style="width:160px;margin-bottom:5px;display:block"> ' . htmlentities($match[0]) . ' </span><iframe style="width:160px;height:140px" src="https://www.youtube.com/embed/' . htmlentities($match[1]) . '" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>', $text);
		}
		...
		if ($giphy_chatroom_off != 1 OR $giphy_off != 1)
		{
			...
			{
				$text = str_replace($match[0], '<div class="arrowchat_giphy_message"><img class="arrowchat_lightbox" data-id="' . htmlentities($match[2]) . '" src="' . htmlentities($match[2]) . '" alt="" style="width:179px" /></div>', $text);
			}
		}
