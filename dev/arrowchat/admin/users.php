		...
		if (hash_equals($_SESSION['token'], $security_token)) 
		{
			...
			if (var_check('group_submit')) 
			{
				...
				$result = $db->execute("
					...
					END WHERE config_name IN ('group_disable_arrowchat', 'group_disable_video', 'group_disable_apps', 'group_disable_rooms', 'group_disable_uploads', 'group_disable_sending_private', 'group_disable_sending_rooms', 'group_is_admin', 'group_is_mod')
				");
