		...
		if (hash_equals($_SESSION['token'], $security_token))
		{
			...
			if (var_check('config_submit')) 
			{
				...
				$result = $db->execute("
					...
					END WHERE config_name IN ('channel_name', 'base_url', 'login_url', 'online_timeout', 'disable_smilies', 'heart_beat', 'buddy_list_heart_beat', 'idle_time', 'push_on', 'push_ssl', 'push_publish', 'push_subscribe', 'spam_protection')
				");
