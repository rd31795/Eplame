                    ...
                    <div class="row">
                        @if(!empty($videos[0]->id))
                            @foreach($videos as $video)
                                @if($video->discussionFileUserId->profile_image)
                                  <?php $src = asset('').'/'.$video->discussionFileUserId->profile_image;
                                  ?>
                                @else
                                ...
                                <div class="col-lg-4 col-md-4 col-sm-6 col-12">
                                    <div class="outer-wrap">
                                        ...
                                        <a class="cstm-modal-anc" href="javascript:void(0)" data-toggle="modal" data-target="#exampleModal" data-src="{{ $video->path }}" data-title="{{ $video->title }}" data-description="{{ $video->description }}" data-user_pic="{{ $src }}" data-user_name="{{ $video->discussionFileUserId->first_name }}" data-upload_date="{{ $video->updated_at }}">
                                      <h5>{{ $video->title }}</h5></a>
    ...
    <script>
        jQuery(document).ready(function(){
            $('.cstm-modal-anc').click(function(){
                ...
                $('#pop-vid').html('<iframe  width="500" height="300" src="'+src+'" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
