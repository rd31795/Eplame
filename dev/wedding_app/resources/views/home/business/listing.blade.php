...
<script type="text/javascript">
$(".main-category").on("change",(e)=>{ $(`.${e.target.id}`).prop('checked',e.target.checked); $.each($(`.${e.target.id}`), function(key, value) {
...
// }); 
$(".sub-category").on('change',(e)=>{
        let length=$(`.${e.target.dataset.id}.sub-category:checked`).length; 
        length==0 ? $(`#${e.target.dataset.id}`).prop('checked',false):$(`#${e.target.dataset.id}`).prop('checked',true);
        let maincatlength=$(`.sub-category:checked`).length;
        let maincatId=$(`.sub-category`).attr('data-id');
