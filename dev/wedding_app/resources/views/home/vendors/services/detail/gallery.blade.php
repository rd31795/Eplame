						  	 ...
						  	 <div id="Photo-slider" class="flexslider">
						          <ul class="slides">
						          	@php 
						          		...
						          		$limit = getAllValueWithMeta('gallery_expiration_time', 'global-settings');
						          	@endphp
						          	@if($vendor->ImageGallery->count() > 0)
							          	@foreach($vendor->ImageGallery as $img)
								        	   
								            <li>
								            	@if($img->updated_at->diffInDays($now) > $limit)
								            		<span class="error-back">Image is older than {{$limit}} Days</span>
								            	@endif
						         ...
						         <ul class="slides">
                              	@if($vendor->VideoGallery->count() > 0)
						          	...
						          	<?php
						          	 ...
						          	 ?>
						            <li>
						            	@if($video->updated_at->diffInDays($now) > $limit)
						            		<span class="error-back" >Video is older than {{$limit}} Days</span>
						            	@endif
